...
public class UserController {
     ...
     * 우리 학과 학생 목록
     */
    @GetMapping("/security/myStudentList")
    public void myStudentList(
        ...
        List<User> studentList = userDao.listOfCertainMajor(majorCode, "student", offset, count);
        int totalCount = userDao.countAllUsers();
...
//        model.addAttribute("majorCode", majorCode);
    ...
    public String preAddSubject(Model model, HttpServletRequest request){
        String majorCode = request.getParameter("majorCode");
        List<User> professorList = userDao.listOfCertainMajor(majorCode, "staff", 0, 100);
